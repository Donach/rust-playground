...
async fn handle_client(sender_stream: TcpStream, clients: Arc<Mutex<HashMap<SocketAddr, TcpStream>>>) {
    ...
    for (&addr, &ref stream) in clients.iter() {
        let res = if addr != sender_stream.peer_addr().unwrap() {
            let (_, writer) = tokio::io::split(stream.to_owned());
            let result = write_to_stream(writer, &msg_type).await;
            match result {
                Ok(_) => true,
                Err(e) => {
