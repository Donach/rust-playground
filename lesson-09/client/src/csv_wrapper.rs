...
pub fn handle_file_to_string(filename: String) -> Result<MessageType, Box<dyn Error>> {
    ...
    match file.read_to_end(&mut input) {
        Ok(_) => Ok(&input),
        Err(error) => {
            eprintln!("Failed to read file: {}", error); Err(Box::new(error))
        }
    };
    
    let path = Path::new(filename.as_str());
    Ok(MessageType::File(path.file_name().unwrap().to_str().unwrap().to_string(), input))
}
